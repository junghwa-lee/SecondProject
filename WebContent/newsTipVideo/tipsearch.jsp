...
<div class="w3-bar">
 <c:if test="${curpage>BLOCK }">
  <a href="../newsTipVideo/tipSearch.do?page=${startPage-1 }&searchkey=${searchkey }" class="w3-button">&lt;</a>
 </c:if>
 <c:forEach var="i" begin="${startPage }" end="${endPage }">
  <c:if test="${i==curpage }">
  <a href="../newsTipVideo/tipSearch.do?page=${i }&searchkey=${searchkey }" class="w3-button w3-blue">${i }</a>
  </c:if>
  <c:if test="${i!=curpage }">
  <a href="../newsTipVideo/tipSearch.do?page=${i }&searchkey=${searchkey }" class="w3-button">${i }</a>
  </c:if>
  </c:forEach>
  <c:if test="${endPage<totalpage }">
  <a href="../newsTipVideo/tipSearch.do?page=${endPage+1 }&searchkey=${searchkey }" class="w3-button">&gt;</a>
  </c:if>
</div>
