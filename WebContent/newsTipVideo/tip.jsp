...
<div class="mySlides fade">
  <img src="${vo.t_poster }" width="600" height="200">
  <br>
  <br>
  <br>
  <br>
  <br>
  <div class="text"><span class="label label-danger">Hot</span> <b>이 시각 인기 꿀팁</b><br>${vo.t_title }</div>
</div>
</a>
</c:forEach>
</div>
...
<span style="text-align: left; height:50px; padding:0px 5px 50px">총 <span style="color:#336699; font-weight:bold;">${tipcount }</span>개</span>
 ...
 <li>
  <a href="../newsTipVideo/tipdetail.do?no=${vo.t_no }">
   ...
   <dl>
    ...
    <dd class="tx">${vo.t_precontent }</dd>
    <dd class="listSubItem">
         <span class="item">${vo.t_regdate }</span>
         <span class="item viewItem"><span class="stSpBefore stIcnView"><span class="skip">&nbsp;&nbsp;|&nbsp;&nbsp;<i class="glyphicon glyphicon-user" style="color:#336699"></i> </span>${vo.t_hit }</span></span>
    </dd>
   </dl>
  </a>
 </li>
...
<div class="topnav">
  ...
  <div class="search-container">
    <form action="../newsTipVideo/tipSearch.do">
...
<div class="w3-bar">
 <c:if test="${curpage>BLOCK }">
  <a href="../newsTipVideo/tip.do?page=${startPage-1 }" class="w3-button">&lt;</a>
 </c:if>
 <c:forEach var="i" begin="${startPage }" end="${endPage }">
  <c:if test="${i==curpage }">
  <a href="../newsTipVideo/tip.do?page=${i }" class="w3-button w3-blue">${i }</a>
  </c:if>
  <c:if test="${i!=curpage }">
  <a href="../newsTipVideo/tip.do?page=${i }" class="w3-button">${i }</a>
  </c:if>
  </c:forEach>
  <c:if test="${endPage<totalpage }">
  <a href="../newsTipVideo/tip.do?page=${endPage+1 }" class="w3-button">&gt;</a>
  </c:if>
</div>
